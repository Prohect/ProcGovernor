...
ProcGovernor 是一个用 Rust 编写的 Windows 服务，它通过 INI 风格的配置文件管理进程 CPU 亲和性、优先级、IO 优先级、内存优先级和线程调度。它支持配置热重载、基于 ETW 的响应式进程检测、带迟滞的 Prime 线程调度，以及用于 Process Lasso 配置转换的工具。
